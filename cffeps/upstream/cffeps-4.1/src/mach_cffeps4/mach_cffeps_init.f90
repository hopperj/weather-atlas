...
!
!
integer function mach_cffeps_init(hotspotfile, feps_inp, nhotspots, aqm_aerobins)
   ...
   use mach_cffeps_fbp_mod,         only: FMCcalc 
...
!   real(kind=4)    :: fmc      ! FMC
   ...
   ! Expanded FBP inputs
   real(kind=4)    :: zs, gfl, gs, aspect, sh, curing, pc, pdf, cbh, &
                      sfl, cfl, ffl, wfl, lfl, dfl, sd  
...
!*** way it is done for the online (GEM-MACH integrated) version
    ...
    do i = 1, nhotspots
       read(funit, 30) lat, lon, ddate, dtime, fuel, estarea, ffmc, dmc, dc,     &
                       curing, zs, gs, aspect, pc, pdf, cbh, gfl, lfl, dfl, sfl, &
                       cfl, ffl, wfl, sh, sd
...
!       the BUI effect is being used. */  --- updated fix 2015-11-09 KRA
       ...
       feps_inp(i)%dtime    = real(dtime / 100) + real(mod(dtime, 100)) / 60.0
       feps_inp(i)%dj       = dj
       ...
       feps_inp(i)%sd       = sd
       feps_inp(i)%sh       = sh
       feps_inp(i)%aspect   = aspect
       feps_inp(i)%curing   = curing
       feps_inp(i)%pc       = pc
       feps_inp(i)%pdf      = pdf
       feps_inp(i)%cbh      = cbh
       feps_inp(i)%cfl      = cfl
       ...
       if (cfl > 0.0) then
          feps_inp(i)%fmc   = FMCcalc(dj, lat, lon, zs)
       else
          feps_inp(i)%fmc   = 120.0
       end if
!
! Initialize other feps_type components that are not read/computed from input
    ...
    close(funit)
 30 format(2(F10.4, 1X), I8, 1X, I4, 1X, A8, 20(1X, F10.4))
! 30 format(F6.3, 4(1X, F8.3, 1X), A8, 1X, F8.3, 1X, I8, 1X, I4, 17(1X, F8.3))
!
!***********************************************************************
!
