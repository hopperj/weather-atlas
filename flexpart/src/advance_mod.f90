...
subroutine advance(itime,ipart,ithread)
  ...
  ! interp_state%ngrid              index which grid is to be used                          *
  ...
  !                    wind, vertical wind                                     *
  ! interp_state%usig,interp_state%vsig,interp_state%wsig     mesoscale wind fluctuations                             *
  ...
  ! Convert z(eta) to z(m) for the turbulence scheme, interp_state%w(m/s) 
  ...
  call interpol_htropo_hmix(tropop,turb_state%h)
  turb_state%zeta=real(part(ipart)%z)/turb_state%h
  ...
  if (turb_state%zeta.le.1.) then
    ...
    if (lsettling) then
      ...
      interp_state%weta=interp_state%weta+weta_settling
    endif
#endif
  endif 
  ...
  if (lturbulence.eq.1) then 
    ...
    if (lmesoscale_turb) then
#ifdef ETA
      ztseta=real(part(ipart)%zeta)
      wsigeta_tmp=interp_state%wsigeta
#else
      ztseta=0.
      wsigeta_tmp=0.
#endif
      ...
      call turbulence_mesoscale(nrand,dxsave,dysave,ipart, &
        interp_state%usig,interp_state%vsig,interp_state%wsig,wsigeta_tmp,eps_eta)
    endif

    !*************************************************************
    ! Transform along and cross wind components to xy coordinates,
    ! add them to interp_state%u and interp_state%v, transform interp_state%u,interp_state%v to grid units/second
  ...
  ! The Petterssen scheme can only applied with long time steps (only then interp_state%u
 ...
 !******************************************************************************
  ...
  if (ngr.ne.interp_state%ngrid) return
...
subroutine adv_above_pbl(itime,itimec,dxsave,dysave,ux,vy,tropop,nrand,ipart)
  ...
  if (mdomainfill.eq.0) then
    if (lsettling) then
      ...
      if (density(nsp).gt.0.) then
        ...
        interp_state%w=interp_state%w+part(ipart)%settling
#endif
      end if
    endif
  end if
  ...
  !************************************************
  dxsave=dxsave+(interp_state%u+ux)*dt
  dysave=dysave+(interp_state%v+vy)*dt

#ifdef ETA
      if (wp.ne.0.) then
        call update_zeta_to_z(itime,ipart)
        call update_z(ipart,wp*dt*real(ldirect))
        if (part(ipart)%z.lt.0.) call set_z(ipart,min(turb_state%h-eps2,-1.*part(ipart)%z))
          ! if particle below ground -> reflection
        call update_z_to_zeta(itime,ipart)
      endif
      call update_zeta(ipart,interp_state%weta*dt*real(ldirect))
      if (part(ipart)%zeta.ge.1.) call set_zeta(ipart,1.-(part(ipart)%zeta-1.))
      if (part(ipart)%zeta.eq.1.) call update_zeta(ipart,-eps_eta)
#else
      call update_z(ipart,(interp_state%w+wp)*dt*real(ldirect))
      if (part(ipart)%z.lt.0.) call set_z(ipart,min(turb_state%h-eps2,-1.*part(ipart)%z))
#endif
...
subroutine adv_in_pbl(itime,itimec, dxsave,dysave,dawsave,dcwsave, abovePBL,  &
  ...
  pbl_loop: do
    ...
    if (loop.eq.1) then ! Temporal interpolation only for the first iteration
      ...
      if (interp_state%ngrid.le.0) then
        ...
        call interpol_pbl(itime,interp_state%xtn,interp_state%ytn,zts,ztseta,ithread+1)
      endif

    else

      ! Determine the level below the current position for interp_state%u,interp_state%v,rho
      !***********************************************************
  ...
  !****************************************************************************
    ...
    dxsave=dxsave+interp_state%u*dt
    dysave=dysave+interp_state%v*dt
    dawsave=dawsave+part(ipart)%turbvel%u*dt
    dcwsave=dcwsave+part(ipart)%turbvel%v*dt
    ! How can I change the interp_state%w to interp_state%w(eta) efficiently?

#ifdef ETA
    call update_z(ipart,interp_state%w*dt*real(ldirect))
    zts=real(part(ipart)%z)
  ...
  !************************************
    ...
    if (zts.lt.0.) call set_z(ipart,min(turb_state%h-eps2,-1.*part(ipart)%z))    
      ! if particle below ground -> reflection
...
subroutine petterssen_corr(itime,ipart)
  ...
  interp_state%u=(interp_state%u-uold)*0.5
  interp_state%v=(interp_state%v-vold)*0.5

#ifdef ETA
    interp_state%weta=(interp_state%weta-woldeta)*0.5
    call update_zeta(ipart,interp_state%weta*real(part(ipart)%idt*ldirect))
    if (part(ipart)%zeta.ge.1.) call set_zeta(ipart,1.-(part(ipart)%zeta-1.))
    if (part(ipart)%zeta.eq.1.) call update_zeta(ipart,-eps_eta)
#else
    interp_state%w=(interp_state%w-wold)*0.5
    call update_z(ipart,interp_state%w*real(part(ipart)%idt*ldirect))
    if (part(ipart)%z.lt.0.) then
      call set_z(ipart,min(turb_state%h-eps2,-1.*part(ipart)%z))
    endif
#endif
  ...
  call update_xy(interp_state%u*part(ipart)%idt,interp_state%v*part(ipart)%idt,ipart)
...
subroutine update_xy(xchange,ychange,ipart)
  ...
  if (interp_state%ngrid.ge.0) then
  ...
  else if (interp_state%ngrid.eq.-1) then      ! around north pole
  ...
  else if (interp_state%ngrid.eq.-2) then    ! around south pole
