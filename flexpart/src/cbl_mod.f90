...
!timedir direction of time forward (1) or backward(-1)
  ...
  ! Use the same signed cube root as cbl().  The transition function can make
  ! skew exactly zero at its lower boundary, and a fractional power is also
  ! undefined for negative skew on some compilers.
  ...
  if (skew.ne.0.) then
    ...
    ! Symmetric-distribution limit.  Evaluating the expression above at zero
    ! forms 0/0 and previously allowed reinitialization itself to return NaN.
    rluarw=0.
    xluarw=0.
  endif
