...
subroutine dealloc_convect
  ...
  if (allocated(convmix_igrid)) deallocate(convmix_igrid)
  if (allocated(convmix_ipoint)) deallocate(convmix_ipoint)
  if (allocated(convmix_igridn)) deallocate(convmix_igridn)
  if (allocated(convmix_frst)) deallocate(convmix_frst)
  if (allocated(convmix_kkcnt)) deallocate(convmix_kkcnt)
...
subroutine ensure_convmix_workspace(alivepart)
  ! Reuse convection sorting/index storage across synchronization steps.
  ! convmix is invoked serially and parallelizes only after this setup.
  implicit none
  integer,intent(in) :: alivepart
  integer :: particle_capacity,column_capacity,stat

  particle_capacity=max(alivepart,1)
  column_capacity=max(nx*(ny+1)+1,1)

  if (.not.allocated(convmix_igrid) .or. &
      size(convmix_igrid).lt.particle_capacity) then
    if (allocated(convmix_igrid)) deallocate(convmix_igrid)
    if (allocated(convmix_ipoint)) deallocate(convmix_ipoint)
    if (allocated(convmix_igridn)) deallocate(convmix_igridn)
    allocate(convmix_igrid(particle_capacity), &
      convmix_ipoint(particle_capacity), &
      convmix_igridn(particle_capacity,max(1,numbnests)),stat=stat)
    if (stat.ne.0) error stop "Could not allocate convection particle workspace"
  endif

  if (.not.allocated(convmix_frst) .or. &
      size(convmix_frst).lt.column_capacity) then
    if (allocated(convmix_frst)) deallocate(convmix_frst)
    if (allocated(convmix_kkcnt)) deallocate(convmix_kkcnt)
    allocate(convmix_frst(column_capacity), &
      convmix_kkcnt(column_capacity),stat=stat)
    if (stat.ne.0) error stop "Could not allocate convection column workspace"
  endif
end subroutine ensure_convmix_workspace
...
subroutine convmix(itime)
  ...
  integer,pointer :: igrid(:), ipoint(:), igridn(:,:)
  ...
  ! OMP changes
  integer :: cnt,kk
  integer,pointer,dimension(:) :: frst,kkcnt
  ...
  call ensure_convmix_workspace(alivepart)
  igrid=>convmix_igrid(1:alivepart)
  ipoint=>convmix_ipoint(1:alivepart)
  igridn=>convmix_igridn(1:alivepart,1:max(1,numbnests))
...
subroutine redist(itime,ipart,ktop,ipconv,ithread)
  ...
  if (levold.ne.nconvtop) then
    ...
    rn=ran3(random_state(ithread-1)%iseed2,ithread-1)
