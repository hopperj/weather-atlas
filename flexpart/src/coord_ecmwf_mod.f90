...
subroutine z_to_zeta(itime,xt,yt,zold,zteta)
  ...
  if (interp_state%ngrid.le.0) then
    do i=2,nz-1
      if ((etauvheight(interp_state%ix,interp_state%jy,i,memind(1)).gt.real(zold)) .or. &
          (etauvheight(interp_state%ixp,interp_state%jy,i,memind(1)).gt.real(zold)) .or. & 
          (etauvheight(interp_state%ix,interp_state%jyp,i,memind(1)).gt.real(zold)) .or. &
          (etauvheight(interp_state%ixp,interp_state%jyp,i,memind(1)).gt.real(zold))) then
        n=i-2
        exit
      endif
    end do
    ...
    do i=2,nz-1
      if ((etauvheightn(interp_state%ix,interp_state%jy,i,memind(1),interp_state%ngrid).gt.real(zold)) .or. &
          (etauvheightn(interp_state%ixp,interp_state%jy,i,memind(1),interp_state%ngrid).gt.real(zold)) .or. & 
          (etauvheightn(interp_state%ix,interp_state%jyp,i,memind(1),interp_state%ngrid).gt.real(zold)) .or. &
          (etauvheightn(interp_state%ixp,interp_state%jyp,i,memind(1),interp_state%ngrid).gt.real(zold))) then
        n=i-2
        exit
      endif
    end do
