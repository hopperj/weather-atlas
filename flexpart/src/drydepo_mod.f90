...
subroutine drydepokernel(nunc,deposit,x,y,nage,kp,thread)
  ...
  ! INT truncates toward zero, which makes a particle just outside the lower
  ! output boundary appear to be in cell zero. The negative fractional
  ! coordinate can then create negative kernel weights and deposition.
  ix=floor(xl)
  jy=floor(yl)
...
subroutine drydepokernel_nest(nunc,deposit,x,y,nage,kp,thread)
  ...
  ! Keep boundary handling consistent with wetdepokernel_nest. INT truncates
  ! negative coordinates toward zero and can create negative kernel weights.
  ix=floor(xl)
  jy=floor(yl)
  ddx=xl-real(ix)                   ! distance to left cell border
  ddy=yl-real(jy)                   ! distance to lower cell border
