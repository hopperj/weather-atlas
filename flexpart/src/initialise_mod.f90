...
subroutine kindz_to_z(ipart)
  ...
  if (kindz(part(ipart)%npoint).eq.3) then
    ...
    do kz=1,nz
      if (interp_state%ngrid.gt.0) then
        r=interp_state%p1*rhon(interp_state%ix ,interp_state%jy ,kz,2,interp_state%ngrid) &
             +interp_state%p2*rhon(interp_state%ixp,interp_state%jy ,kz,2,interp_state%ngrid) &
             +interp_state%p3*rhon(interp_state%ix ,interp_state%jyp,kz,2,interp_state%ngrid) &
             +interp_state%p4*rhon(interp_state%ixp,interp_state%jyp,kz,2,interp_state%ngrid)
        t=interp_state%p1*ttn(interp_state%ix ,interp_state%jy ,kz,2,interp_state%ngrid) &
             +interp_state%p2*ttn(interp_state%ixp,interp_state%jy ,kz,2,interp_state%ngrid) &
             +interp_state%p3*ttn(interp_state%ix ,interp_state%jyp,kz,2,interp_state%ngrid) &
             +interp_state%p4*ttn(interp_state%ixp,interp_state%jyp,kz,2,interp_state%ngrid)
      else
        r=interp_state%p1*rho(interp_state%ix ,interp_state%jy ,kz,2) &
             +interp_state%p2*rho(interp_state%ixp,interp_state%jy ,kz,2) &
             +interp_state%p3*rho(interp_state%ix ,interp_state%jyp,kz,2) &
             +interp_state%p4*rho(interp_state%ixp,interp_state%jyp,kz,2)
        t=interp_state%p1*tt(interp_state%ix ,interp_state%jy ,kz,2) &
             +interp_state%p2*tt(interp_state%ixp,interp_state%jy ,kz,2) &
             +interp_state%p3*tt(interp_state%ix ,interp_state%jyp,kz,2) &
             +interp_state%p4*tt(interp_state%ixp,interp_state%jyp,kz,2)
      endif
      press=r*r_air*t/100.
  ...
  !***********************************************************************
    ...
    if (interp_state%ngrid.gt.0) then
      topo=interp_state%p1*oron(interp_state%ix ,interp_state%jy ,interp_state%ngrid) &
           + interp_state%p2*oron(interp_state%ixp,interp_state%jy ,interp_state%ngrid) &
           + interp_state%p3*oron(interp_state%ix ,interp_state%jyp,interp_state%ngrid) &
           + interp_state%p4*oron(interp_state%ixp,interp_state%jyp,interp_state%ngrid)
    else
      topo=interp_state%p1*oro(interp_state%ix ,interp_state%jy) &
           + interp_state%p2*oro(interp_state%ixp,interp_state%jy) &
           + interp_state%p3*oro(interp_state%ix ,interp_state%jyp) &
           + interp_state%p4*oro(interp_state%ixp,interp_state%jyp)
    endif
...
subroutine init_mass_conversion(ipart,ipoint)
  ...
  if ((ind_rel .eq. 1).or.(ind_rel .eq. 3).or.(ind_rel .eq. 4)) then
    ...
    call find_vert_vars_lin(height,real(part(ipart)%z),interp_state%indz,dz1,dz2,interp_state%lbounds)

    if (interp_state%ngrid.gt.0) then
      do n=1,2
        rhoaux(n)=interp_state%p1*rhon(interp_state%ix ,interp_state%jy ,interp_state%indz+n-1,2,interp_state%ngrid) &
             +interp_state%p2*rhon(interp_state%ixp,interp_state%jy ,interp_state%indz+n-1,2,interp_state%ngrid) &
             +interp_state%p3*rhon(interp_state%ix ,interp_state%jyp,interp_state%indz+n-1,2,interp_state%ngrid) &
             +interp_state%p4*rhon(interp_state%ixp,interp_state%jyp,interp_state%indz+n-1,2,interp_state%ngrid)
      end do
    else
      do n=1,2
        rhoaux(n)=interp_state%p1*rho(interp_state%ix ,interp_state%jy ,interp_state%indz+n-1,2) &
             +interp_state%p2*rho(interp_state%ixp,interp_state%jy ,interp_state%indz+n-1,2) &
             +interp_state%p3*rho(interp_state%ix ,interp_state%jyp,interp_state%indz+n-1,2) &
             +interp_state%p4*rho(interp_state%ixp,interp_state%jyp,interp_state%indz+n-1,2)
      end do
    endif
...
subroutine init_particle(itime,ipart,ithread)
  ...
  ! interp_state%usig,interp_state%vsig,interp_state%wsig     uncertainties of wind velocities due to interpolation   *
  ...
  use turbulence_mod
  use random_mod, only: ran3,random_state
  use omp_lib
  use interpol_mod
  use cbl_mod
  ...
  nrand=int(ran3(random_state(ithread)%iseed1,ithread)*real(maxrand-1))+1
  ...
  turb_state%h=max(hmix(interp_state%ix ,interp_state%jy,1,memind(1)), &
       hmix(interp_state%ixp,interp_state%jy ,1,memind(1)), &
       hmix(interp_state%ix ,interp_state%jyp,1,memind(1)), &
       hmix(interp_state%ixp,interp_state%jyp,1,memind(1)), &
       hmix(interp_state%ix ,interp_state%jy ,1,memind(2)), &
       hmix(interp_state%ixp,interp_state%jy ,1,memind(2)), &
       hmix(interp_state%ix ,interp_state%jyp,1,memind(2)), &
       hmix(interp_state%ixp,interp_state%jyp,1,memind(2)))

  turb_state%zeta=zt/turb_state%h
  ...
  if (turb_state%zeta.le.1.) then

    call interpol_pbl(itime,xt,yt,zt,zteta,ithread+1)

  ! Vertical interpolation of interp_state%u,interp_state%v,interp_state%w,rho and drhodz
  !***********************************************

  ! Vertical distance to the level below and above current position
  ! both in terms of (interp_state%u,interp_state%v) and (interp_state%w) fields
  !****************************************************************
  ...
  !*****************************************
    ...
    part(ipart)%turbvel%u=rannumb(nrand)*turb_state%sigu
    part(ipart)%turbvel%v=rannumb(nrand+1)*turb_state%sigv
    part(ipart)%turbvel%w=rannumb(nrand+2)
    if (.not.turbswitch) then     ! modified by mc
      part(ipart)%turbvel%w=part(ipart)%turbvel%w*turb_state%sigw
    else if (cblflag.eq.1) then   ! modified by mc
      if(-turb_state%h/turb_state%ol.gt.5) then
  !if (turb_state%ol.lt.0.) then
  !if (turb_state%ol.gt.0.) then !by mc : only for test correct is lt.0
        call init_cbl_vel( &
          random_state(ithread)%iseed1,zt,turb_state%wst,turb_state%h, &
          turb_state%sigw,part(ipart)%turbvel%w,turb_state%ol,ithread)
      else
        part(ipart)%turbvel%w=part(ipart)%turbvel%w*turb_state%sigw
      end if
    end if
  ...
  !*****************************************
    ...
    if (turbswitch) then
      part(ipart)%idt = int( &
        min(turb_state%tlw, &
          turb_state%h/max(2.*abs(part(ipart)%turbvel%w*turb_state%sigw),1.e-5), &
          0.5/abs(turb_state%dsigwdz), &
          600.) &
        *ctl)
    else
      part(ipart)%idt = int( &
        min(turb_state%tlw, &
          turb_state%h/max(2.*abs(part(ipart)%turbvel%w),1.e-5), &
          600.) &
        *ctl)
    endif
    ...
    ! interp_state%usig=(usigprof(interp_state%indzp)+usigprof(interp_state%indz))/2.
    ! interp_state%vsig=(vsigprof(interp_state%indzp)+vsigprof(interp_state%indz))/2.
    ! interp_state%wsig=(wsigprof(interp_state%indzp)+wsigprof(interp_state%indz))/2.

    ! interp_state%wsigeta=(wsigprofeta(interp_state%indzpeta)+wsigprofeta(interp_state%indzeta))/2.
  ...
  if (lmesoscale_turb) then
    ...
    if (nrand+2.gt.maxrand) nrand=1
    part(ipart)%mesovel%u=rannumb(nrand)*interp_state%usig
    part(ipart)%mesovel%v=rannumb(nrand+1)*interp_state%vsig
#ifdef ETA
    part(ipart)%mesovel%w=rannumb(nrand+2)*interp_state%wsigeta
#else
    part(ipart)%mesovel%w=rannumb(nrand+2)*interp_state%wsig
#endif
  endif
