...
module readoptions_mod
  ...
  implicit none
  logical :: DRYDEP_ENABLED=.true.,WETDEP_ENABLED=.true.
...
subroutine readreleases
  ...
  ! These user switches are applied after the species physics have been
  ! initialised.  In particular, disabling dry deposition must not erase
  ! particle density/diameter or the gravitational-settling calculation.
  if (.not.WETDEP_ENABLED) then
    WETDEP=.false.
    WETDEPSPEC(:)=.false.
    if (lroot) write (*,*) '  Wet deposition disabled by COMMAND'
  endif
  if (.not.DRYDEP_ENABLED) then
    DRYDEP=.false.
    DRYDEPSPEC(:)=.false.
    if (lroot) write (*,*) '  Dry deposition disabled by COMMAND; settling retained'
  endif
  DEP=WETDEP.or.DRYDEP
