...
!  use ohr_mod
  ...
  real(dep_prec),allocatable,dimension(:,:) ::       &
    drytmp_threads       ! reusable dry-deposition scratch, species x thread
  logical :: itsopen
...
!  recoutnumsat(:,:)=0.
  ...
  if (DRYDEP) then
    allocate(drytmp_threads(maxspec,numthreads_grid),stat=stat)
    if (stat.ne.0) error stop "Could not allocate dry-deposition thread workspace"
    drytmp_threads=0._dep_prec
  endif
...
!$OMP PARALLEL PRIVATE(prob_rec,nage,inage,itage,ks,kp,thread,i,j,xmassfract)
...
!max(1,int(real(numpart)/numthreads/20.)))
    do i=1,count%alive
      ...
      j=count%ialive(i)
      if (DRYDEP) drytmp_threads(:,thread+1)=0._dep_prec
      ...
      if (DRYDEP.or.WETDEP.or.LDECAY) then
        ...
        do ks=1,nspec
          ...
          if (DRYDEPSPEC(ks)) then     ! dry deposition (and radioactive decay)
            ...
            call drydepo_massloss(j,ks,ldeltat,drytmp_threads(ks,thread+1))
...
!        don't create depofield for backward runs
      if (DRYDEP.AND.(ldirect.eq.1).and.(iout.ne.0)) then
        ...
        call drydepokernel(part(j)%nclass,drytmp_threads(:,thread+1),real(part(j)%xlon), &
             real(part(j)%ylat),nage,kp,thread+1)
        if (nested_output.eq.1) call drydepokernel_nest( &
             part(j)%nclass,drytmp_threads(:,thread+1),real(part(j)%xlon),real(part(j)%ylat), &
             nage,kp,thread+1)
      endif
...
!$OMP END PARALLEL
  ...
  if (allocated(drytmp_threads)) deallocate(drytmp_threads)
