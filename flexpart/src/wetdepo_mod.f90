...
subroutine get_wetscav(itime,jpart,ks,gridfract,wetscav)
  ...
  if ( (interp_state%ngrid.gt.0) ) then
    if (lcw_nest(interp_state%ngrid)) readclouds_this_nest=.true.
  endif
...
subroutine get_cloud_liquid(gridfract,prec,cc,cl)
  ...
  if (interp_state%ngrid.le.0) then
    ...
    if (lcw_nest(interp_state%ngrid)) then
      if (lgridfraction) then
        cl=cl/rho_water*(gridfract/cc)
      else
        cl=cl/rho_water ! Grythe et al. eq (1), no cloud_thickness since this will cancel out later
...
subroutine wetdepokernel(nunc,deposit,x,y,nage,kp,thread)
  ...
  ! As in the nested-grid kernel, use the mathematical containing cell.
  ! INT truncation can create negative weights outside the lower boundary.
  ix=floor(xl)
  jy=floor(yl)
