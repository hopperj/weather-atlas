...
run_id="noaa_gfs__$(date -u +%Y%m%dT%H%M%SZ)__$$"
wait_timeout=${GFS_WAIT_TIMEOUT_SECONDS:-7200}
poll_interval=${GFS_POLL_INTERVAL_SECONDS:-5}
if [[ ! "$wait_timeout" =~ ^[1-9][0-9]*$ ]] \
  || [[ ! "$poll_interval" =~ ^[1-9][0-9]*$ ]]; then
  echo "GFS wait and poll intervals must be positive integers." >&2
  exit 2
fi
